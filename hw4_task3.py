...
def visual_directory_structure(path: pathlib.Path, indent: str = ""):
    ...
    for item in path.iterdir():
        if item.is_dir():
            ...
            visual_directory_structure(item, indent + "    ")
        else:
            print(Fore.GREEN + f"{indent}{item.name}")


# Example usage:
# python hw4_task3.py /path/to/your/directory
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(
            Fore.RED
            + "Please use next format to run the script: python hw3_task3.py /path/to/directory"
        )
        sys.exit(1)  # Exit the script with an error code
    # Get the directory path from the command line argument
    dir_path = pathlib.Path(sys.argv[1])
    # Print the directory structure
    visual_directory_structure(dir_path)
